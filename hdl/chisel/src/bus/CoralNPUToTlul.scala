...
object CoralNPUToTlul {
  object State extends ChiselEnum {
    val sIdle, sWaitForReady, sWaitForResponse = Value
  }

  def apply(tlul_p: TLULParameters, coralnpu_p: coralnpu.Parameters): CoralNPUToTlul = {
    return Module(new CoralNPUToTlul(tlul_p, coralnpu_p))
  }
}

class CoralNPUToTlul(tlul_p: TLULParameters, coralnpu_p: coralnpu.Parameters) extends Module {
  import CoralNPUToTlul.State._

  val io = IO(new Bundle {
    val tl_i = Input(new TileLinkULIO_D2H(tlul_p))
    val tl_o = Output(new TileLinkULIO_H2D(tlul_p))
    val coralnpu = Flipped(new CoralNPUMemIO(coralnpu_p))
  })
  val state = RegInit(sIdle)

  val wmask_width = io.coralnpu.wmask.getWidth
  val wmask_bits = (0 until wmask_width)
    .map(x => Mux(io.coralnpu.wmask(x), 0xff.U(wmask_width.W) << (x * 8).U, 0.U(wmask_width.W)))
    .reduce(_ | _)
  ...
  io.coralnpu.rid := 0.U
  io.coralnpu.cready := true.B
  io.coralnpu.rvalid := false.B
  io.coralnpu.rdata := 0.U
  ...
  switch(state) {
    is(sIdle) {
      io.coralnpu.cready := true.B
      when(io.coralnpu.cvalid) {
        io.coralnpu.cready := false.B
        io.tl_o.a_valid := true.B
        io.tl_o.a_address := io.coralnpu.caddr
        val cwrite = io.coralnpu.cwrite
        io.tl_o.a_opcode := Mux(cwrite, TLULOpcodesA.PutFullData.asUInt, TLULOpcodesA.Get.asUInt)
        io.tl_o.a_data := Mux(cwrite, io.coralnpu.wdata & wmask_bits, 0.U)
    ...
    is(sWaitForResponse) {
      ...
      when(io.tl_i.d_valid) {
        ...
        val rid = chisel3.util.MuxLookup(value, 0.U)(
          Seq(
            TLULOpcodesD.AccessAck -> 0.U,
            TLULOpcodesD.AccessAckData -> io.coralnpu.cid
          )
        )
        io.coralnpu.rvalid := rvalid
        io.coralnpu.rdata := rdata
        io.coralnpu.cready := true.B
        io.coralnpu.rid := rid
        state := sIdle
      }
