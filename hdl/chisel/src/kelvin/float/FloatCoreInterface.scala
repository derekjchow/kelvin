...
package kelvin.float

import common.{MakeWireBundle, MakeValid, MakeInvalid}
import chisel3._
import chisel3.util._
import kelvin.{FRegfileRead, FRegfileWrite, RegfileReadDataIO, RegfileWriteDataIO, Parameters}
...
object FloatInstruction {
  def decode(inst: UInt, addr: UInt): Valid[FloatInstruction] = {
    ...
    val opcode = MuxLookup(in_opcode, MakeInvalid(FloatOpcode()))(Seq(
        "b00001".U -> MakeValid(FloatOpcode.LOADFP),
        "b01001".U -> MakeValid(FloatOpcode.STOREFP),
