...
class Lsu(p: Parameters) extends Module {
  val io = IO(new Bundle {
    // Decode cycle.
    val req = Vec(4, Flipped(Decoupled(new LsuCmd)))
  ...
  for (i <- 0 until 4) {
    ...
    val opsize = Cat(io.req(i).bits.op.isOneOf(LsuOp.LW, LsuOp.SW, LsuOp.LH, LsuOp.LHU,
                                               LsuOp.SH, LsuOp.LB, LsuOp.LBU, LsuOp.SB))
