...
class SCore(p: Parameters) extends Module {
  ...
  if (p.enableRvv) {
    for (i <- 0 until p.instructionLanes) {
      ...
      // io.rvvcore.get.inst(i).valid := false.B
      // io.rvvcore.get.inst(i).bits := 0.U.asTypeOf(io.rvvcore.get.inst(i).bits)

      // decode(i).io.rvv.get.ready := false.B  // TODO(derekjchow): Fix me
      // assert(!decode(i).io.rvv.get.valid)
    }
  }
